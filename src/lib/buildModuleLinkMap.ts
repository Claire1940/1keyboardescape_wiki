...
function matchScore(queryText: string, article: ArticleWithType, extraKeywords?: string[]): number {
  ...
  // Exact phrase match in title (stripped of "+1 Speed Keyboard Escape")
  const strippedQuery = normalizedQuery.replace(/1 speed keyboard escape\s*/g, '').trim()
  const strippedTitle = normalizedTitle.replace(/1 speed keyboard escape\s*/g, '').trim()
